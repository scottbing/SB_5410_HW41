...
class Application(Frame):
    ...
    def create_widgets(self):
        ...
        Label(self, text='If you click the button, the text in the entry widget will show above.').grid(row=11, column=0, sticky=W, pady=4)

        # this is a variable Entry widget whose content is saved into `self.text_input`
        Entry(self, textvariable=self.text_input).grid(row=13, column=0, sticky=W, pady=4)

        # this is the button who will trigger `self.set_msg` on click.
        Button(self, text='Search', command=self.set_msg).grid(row=14, column=0, sticky=W, pady=4)

        # this is a variable Label widget whose display is decided by the linked variable `self.msg2show`
        Label(self, textvariable=self.msg2show).grid(row=12, column=0, sticky=W, pady=4)
    ...
    def getStates(self):
        ...
        print(params)
        #self.results_frame.['text'] = params
