...
def process_file(fname, enc):
    ...
    with open(fname, 'r', encoding=enc) as file:
        ...
        dat = perform_re(dat)
    return(dat)  #return read data !-deleted split-!
...
def main():
    ...
    words = process_file("alice.txt", "utf-8")

    #use nltk tokenizer (like split)
    nltk_words = word_tokenize(words)

    words = words.split()   #need to spit doewn here now.
    print("{0} vs {1}".format(len(nltk_words), len(words)))
