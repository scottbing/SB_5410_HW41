...
class Application(Frame):
    ...
    def create_widgets(self):
        ...
        ttk.Separator(self).grid(row=9, column=0, columnspan=99, sticky=EW, pady=7)

        Label(self,
              text="Choose one story above to search",
              font=("Helvetica", 14)
              ).grid(row=10, column=0, sticky=W, pady=3)
        ...
        Entry(self,
              width=40
              ).grid(row=11, column=0, sticky=E, pady=3)

        # create a the generate button
        Button(self,
               text="Search",
               command=self.search,
               font=btnFont).grid(row=12, column=0, sticky=E)
